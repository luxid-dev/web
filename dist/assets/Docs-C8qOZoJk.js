import{c as R,u as T,b as B,r as j,j as e,X as V,L as P,a as Q,R as Z}from"./index-DHSAVma5.js";import{d as M,e as ee,g as J,H as se,S as te,F as ne,f as re,h as H,i as Y,j as G}from"./Footer-CV8dSw2u.js";import{A as ie,b as n}from"./CodeExample-0Ge9s6jR.js";import{E as ae}from"./external-link-DWw7vwpA.js";/**
...
cd blog`}),e.jsx(r,{children:"The tour"}),e.jsx(t,{children:"Nine things were created. Here is what each is for, in the order a request touches them."}),e.jsx(d,{children:e.jsx(s,{children:"src/main.rs"})}),e.jsx(n,{language:"rust",code:`mod app;
mod config;
mod controllers;
...
});`}),e.jsxs(t,{children:[e.jsx("strong",{children:"Route"})," — one endpoint:"]}),e.jsx(n,{language:"rust",code:'r.get("/me", MeController::show).middleware(Auth::jwt());'}),e.jsx(t,{children:"Or across a whole resource:"}),e.jsx(n,{language:"rust",code:'r.resource("/posts", PostsController).middleware(Auth::jwt());'}),e.jsx(t,{children:"Several at once:"}),e.jsx(n,{language:"rust",code:'r.get("/admin", AdminController::show).middleware((Auth::jwt(), Role::admin()));'}),e.jsxs(t,{children:["Middleware is attached by ",e.jsx("strong",{children:"value"}),", not by a string name, so a typo is a compile error rather than a route that silently runs unguarded."]}),e.jsx(r,{children:"Order"}),e.jsx(t,{children:"Middleware runs outermost first: global, then group, then route. On the way out it unwinds in reverse."}),e.jsxs(t,{children:["With ",e.jsx(s,{children:"Timer"})," global and ",e.jsx(s,{children:"Auth::jwt()"})," on a group:"]}),e.jsx(n,{language:"text",code:`→ Timer starts
...
Post::count_all().await?        // u64`}),e.jsxs(t,{children:[e.jsx(s,{children:"find_or_fail"})," is the one you will use most, because it makes actions short:"]}),e.jsx(n,{language:"rust",code:`async fn show(ctx: HttpContext) -> Result<Response> {
    let post = Post::find_or_fail(ctx.params.get::<i64>("id")?).await?;
    ctx.response.ok(post)
...
└── post.rs      StorePost, UpdatePost`}),e.jsxs(t,{children:[e.jsx(s,{children:"luxid make:model User -a"})," generates the file with both structs and empty rule lists. ",e.jsx(s,{children:"cargo luxid db:sync"})," can refresh the field list from the schema, and touches only what lies between the markers — the rules you wrote survive."]}),e.jsx(r,{children:"Building errors by hand"}),e.jsx(t,{children:"Occasionally a rule does not fit the declarative form:"}),e.jsx(n,{language:"rust",code:`async fn store(ctx: HttpContext) -> Result<Response> {
...
async fn the_index_is_paginated(db: Db) -> Result<()> {
    ...
    Ok(())
}`}),e.jsxs(t,{children:["Add the harness to your ",e.jsx(s,{children:"Cargo.toml"}),":"]}),e.jsx(n,{language:"toml",code:`[dev-dependencies]
...
curl -H "$AUTH" 'localhost:3000/api/todos?status=done'`}),e.jsx(t,{children:"And confirm the guard:"}),e.jsx(n,{language:"sh",code:"curl localhost:3000/api/todos        # 401"}),e.jsx(r,{children:"9. Loading the owner"}),e.jsx(t,{children:"To include the owner in a response:"}),e.jsx(n,{language:"rust",code:`let todos = Todo::owned_by(user_id)
    ...
    .await?;`}),e.jsx(n,{language:"json",code:'{ "id": 1, "title": "Write the docs", "owner": { "id": 1, "name": "Ada" } }'}),e.jsxs(t,{children:["One extra query for the whole page, not one per row. And if you forget the ",e.jsx(s,{children:'.with("owner")'})," but call ",e.jsx(s,{children:"todo.owner()?"}),", development tells you:"]}),e.jsx(n,{language:"text",code:'the `owner` relation of `Todo` was not loaded. Add `.with("owner")` to the query.'}),e.jsx(t,{children:"That is the N+1 protection from chapter 13 doing its job."}),e.jsx(r,{children:"10. Tests"}),e.jsxs(t,{children:[e.jsx(s,{children:"tests/todos.rs"}),":"]}),e.jsx(n,{language:"rust",code:`use luxid::prelude::*;
use luxid_testing::TestApp;
use sea_orm::ActiveValue::Set;
use serde_json::json;
